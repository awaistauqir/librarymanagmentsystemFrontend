...
const EbookDetail = ({ asset }: Props) => {
  ...
  const PDF = asset.pdf
    ? `${process.env.NEXT_PUBLIC_BASE_URL}${asset.pdf
        ?.replace("\\", "/")
        .replace("\\", "/")}`
    : "";
  return (
    <div>
      ...
      <div className="flex justify-end p-4 space-x-4">
        <Button variant="outlined" LinkComponent={Link} href="/ebooks">
          Back
        </Button>
        <Button
          variant="contained"
          ...
          href={PDF}
          target="_blank"
          disabled={typeof asset.pdf !== "string"}
        >
          Read
        </Button>
      </div>
