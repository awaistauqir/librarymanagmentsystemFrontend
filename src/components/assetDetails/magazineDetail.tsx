...
const MagazineDetail = ({ asset }: Props) => {
  ...
  return (
    <div>
      ...
      <div className="flex space-x-4 justify-start items-center">
        ...
        <div>
          ...
          <h1 className="font-bold text-red-500">
            Volume No.: <span>{asset.volume_no}</span>
          </h1>

          <h1 className="font-bold">
            Lanugage: <span>{asset.language.name}</span>
          </h1>
          <h1
            className={`font-bold ${
      ...
      <div className="flex justify-end p-4 space-x-4">
        <Button variant="outlined" LinkComponent={Link} href="/magazines">
          Back
        </Button>
        {!asset.is_available && (
          <Button
            variant="contained"
            LinkComponent={Link}
            href={`/magazines/reissuance/${asset.id}`}
          >
            Re-issue
          </Button>
        )}
        {asset.is_available && (
          <Button
            variant="contained"
            LinkComponent={Link}
            href={`/magazines/issuance/${asset.id}`}
          >
            Issue
          </Button>
        )}
        {!asset.is_available && (
          <Button
            variant="contained"
            LinkComponent={Link}
            href={`/magazines/return/${asset.id}`}
          >
            Return
          </Button>
        )}
