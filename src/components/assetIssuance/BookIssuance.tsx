...
const BookIssuance = (props: Props) => {
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      ...
      try {
        ...
        router.replace(`/books/${props.issuing_asset.id}`);
      } catch (error) {
        Swal.fire({
          title: "Sucess",
          text: "Book Issued Successfully",
      ...
      <form className="space-x-4 space-y-4" onSubmit={formik.handleSubmit}>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <TextField
            ...
            size="small"
            fullWidth
            value={props.issuing_asset.acc_no}
          />
          <TextField
            label="Title"
            size="small"
        ...
        <div className="flex justify-end items-center space-x-4">
          <Button variant="outlined" href={`/books/${props.issuing_asset.id}`}>
            Cancel
          </Button>
