...
const MagazineReissuance = (props: Props) => {
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      ...
      try {
        ...
        Swal.fire({
          title: "Sucess",
          text: "Magazine Issued Successfully",
          icon: "success",
        });
        router.replace(`/magazines/${props.borrowedItem.issued_asset.id}`);
      } catch (error) {
  ...
  return (
    <div>
      ...
      <form className="space-x-4 space-y-4" onSubmit={formik.handleSubmit}>
        <h2 className="text-lg font-bold">Magazine Reissuance</h2>
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <TextField
            ...
            size="small"
            fullWidth
            value={props.borrowedItem.issued_asset.volume_no}
          />
        </div>
        ...
        <div className="flex justify-end items-center space-x-4">
          <Button
            variant="outlined"
            href={`/magazines/${props.borrowedItem.issued_asset.id}`}
          >
            Cancel
          </Button>
...
export default MagazineReissuance;
