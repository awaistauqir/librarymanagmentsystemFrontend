  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      ...
      try {
        ...
        Swal.fire({
          title: "Sucess",
          text: "Novel Returned Successfully",
          icon: "success",
        });
        router.replace(`/novels/${props.returnAsset.issued_asset.id}`);
      } catch (error) {
        if (error instanceof AxiosError) {
          Swal.fire({
            title: "Error",
            text: error.response?.data.message || "Novel return failed",
            icon: "error",
          });
        }
      } finally {
      ...
      <form className="space-x-4 space-y-4" onSubmit={formik.handleSubmit}>
        ...
        <div className="flex justify-end items-center space-x-4">
          <Button
            variant="outlined"
            LinkComponent={Link}
            href={`/novels/${props.returnAsset.issued_asset.id}`}
          >
            Cancel
          </Button>
