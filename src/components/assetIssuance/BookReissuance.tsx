...
const BookReissuance = (props: Props) => {
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      ...
      try {
        ...
        router.replace(`/books/${props.borrowedItem.issued_asset.id}`);
      } catch (error) {
  ...
  return (
    <div>
      ...
      <form className="space-x-4 space-y-4" onSubmit={formik.handleSubmit}>
        <h2 className="text-lg font-bold">Book Reissuance</h2>
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <TextField
            label="Acc No."
            size="small"
            fullWidth
            value={props.borrowedItem.issued_asset.acc_no}
          />
          <TextField
            label="Title"
            size="small"
        ...
        <div className="flex justify-end items-center space-x-4">
          <Button
            variant="outlined"
            href={`/books/${props.borrowedItem.issued_asset.id}`}
          >
            Cancel
          </Button>
