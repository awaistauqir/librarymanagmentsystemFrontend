...
const MagazineReturn = (props: Props) => {
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      ...
      try {
        ...
        Swal.fire({
          title: "Sucess",
          text: "Magzine Returned Successfully",
          icon: "success",
        });
        router.replace(`/magazines/${props.returnAsset.issued_asset.id}`);
      } catch (error) {
        if (error instanceof AxiosError) {
          Swal.fire({
            title: "Error",
            text: error.response?.data.message || "Magzine return failed",
            icon: "error",
          });
        }
      } finally {
  ...
  return (
    <div>
      ...
      <form className="space-x-4 space-y-4" onSubmit={formik.handleSubmit}>
        <h2 className="text-lg font-bold">Magzine Details</h2>
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <TextField
            ...
            size="small"
            fullWidth
            value={props.returnAsset.issued_asset.volume_no}
          />
        </div>
        ...
        <div className="flex justify-end items-center space-x-4">
          <Button
            variant="outlined"
            LinkComponent={Link}
            href={`/magazines/${props.returnAsset.issued_asset.id}`}
          >
            Cancel
          </Button>
