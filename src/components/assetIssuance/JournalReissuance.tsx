...
const JournalReissuance = (props: Props) => {
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      ...
      try {
        ...
        Swal.fire({
          title: "Sucess",
          text: "Journal Issued Successfully",
          icon: "success",
        });
        router.replace(`/journals/${props.borrowedItem.issued_asset.id}`);
      } catch (error) {
  ...
  return (
    <div>
      ...
      <form className="space-x-4 space-y-4" onSubmit={formik.handleSubmit}>
        <h2 className="text-lg font-bold">Journal Reissuance</h2>
        ...
        <div className="flex justify-end items-center space-x-4">
          <Button
            variant="outlined"
            href={`/journals/${props.borrowedItem.issued_asset.id}`}
          >
            Cancel
          </Button>
...
export default JournalReissuance;
