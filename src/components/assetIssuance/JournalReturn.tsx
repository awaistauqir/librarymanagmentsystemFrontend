...
const JournalReturn = (props: Props) => {
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      ...
      try {
        ...
        Swal.fire({
          title: "Sucess",
          text: "Journal Returned Successfully",
          icon: "success",
        });
        router.replace(`/journals/${props.returnAsset.issued_asset.id}`);
      } catch (error) {
        if (error instanceof AxiosError) {
          Swal.fire({
            title: "Error",
            text: error.response?.data.message || "Journal return failed",
            icon: "error",
          });
        }
      } finally {
  ...
  return (
    <div>
      ...
      <form className="space-x-4 space-y-4" onSubmit={formik.handleSubmit}>
        <h2 className="text-lg font-bold">Journal Details</h2>
        ...
        <div className="flex justify-end items-center space-x-4">
          <Button
            variant="outlined"
            LinkComponent={Link}
            href={`/journals/${props.returnAsset.issued_asset.id}`}
          >
            Cancel
          </Button>
