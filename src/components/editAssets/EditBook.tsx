...
const EditBookForm = ({
  ...
  >(`${process.env.NEXT_PUBLIC_BASE_URL}/${bookRecord.cover}`);
  ...
  const createBookSchema = object().shape({
    ...
    publishing_year: number().typeError("Please enter a valid year"),
    date_of_purchase: date().typeError("Please enter a valid date").nullable(),
    price: number()
  ...
  const formik = useFormik({
    initialValues: {
      ...
      distributerId: bookRecord.distributer?.id || "",
      ...
      currencyId: bookRecord.currency?.id || "",
    ...
    onSubmit: async (values) => {
      ...
      axiosAuth
        .patch(`${BOOKS}/update/${bookRecord.id}`, formData)
        .then(() => {
          Swal.fire({
            title: "Success",
