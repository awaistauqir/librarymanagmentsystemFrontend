...
import { Publisher } from "@/lib/commonInterfaces"; // Assuming this is the correct import for the Publisher interface
...
interface Props {
  publisher: Publisher; // Updated prop name
}

const EditPublisherForm = ({ publisher }: Props) => {
  ...
  const formik = useFormik({
    initialValues: {
      name: publisher.name, // Updated prop name
    },
    validationSchema: schema,
    onSubmit: async (values) => {
      setLoading(false);
      axiosAuth
        .patch(`${PUBLISHERS}/update/${publisher.id}`, trimStrings(values)) // Updated endpoint
        .then(() => {
          Swal.fire({
            title: "Success",
